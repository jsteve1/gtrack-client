    ...
    return (
        ...
        <Row className="goal-list-item-row-sort">
            <Col className="d-flex justify-content-start align-items-center col-sort-by-text">
                ...
                <Button className={`sort-button ${(currentSortState === "deadline") ? "selected-sort-button" : ""}`} variant="dark" onClick={() => { setCurrentSortState("deadline"); setRearrangeMode(false); }}>Deadline</Button>
                {
                    (currentSortState === "priority") ? 
                        <Button variant="dark" className="rearrange-button" onClick={() => setRearrangeMode(!rearrangeMode) }><Icon.FilterLeft />&nbsp;Rearrange</Button> 
                        : ""
            ...
            (rearrangeMode) ? <>
                    ...
                    _goals.map((goal, index) => (
                        <>
                            <Row className="goal-list-item-row" key={goal.id}>
                                <GoalListItem rearrangeMode={true} priority={index + 1} name={goal.name} deadline={goal.deadline} />
                            </Row>
                        </>
                    ))
                }
            </> : <>
                {
                      _goals.map((goal, index) => (
                        <>
                            <Row className="goal-list-item-row" key={goal.id}>
                                <GoalListItem rearrangeMode={false} showPrio={currentSortState !== "deadline"} priority={index + 1} name={goal.name} deadline={goal.deadline} />
                            </Row>
                        </>
                    ))
                }
